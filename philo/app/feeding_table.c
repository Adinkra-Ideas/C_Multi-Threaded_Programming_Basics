...
/*   Updated: 2022/08/05 17:28:41 by euyi             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*philo_deeds(void *vptr)
{
	...
	{
		pthread_mutex_lock(&(((t_philo *)vptr)->philo_mutex[i]));
		echo_eat((t_philo *)vptr, i, 0);
		pthread_mutex_lock(&(((t_philo *)vptr)
				->philo_mutex[rig(((t_philo *)vptr)->philo_count, i)]));
		if (echo_death((t_philo *)vptr, i))
			break ;
		echo_eat((t_philo *)vptr, i, 1);
